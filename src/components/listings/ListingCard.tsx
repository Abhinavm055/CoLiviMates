...
// Helper functions for student-focused badges
const isStudentFriendly = (listing: Listing) => {
  const keywords = ['student', 'college', 'university', 'psg', 'vit', 'anna', 'christ', 'jntu'];
  const text = (listing.title + ' ' + listing.description).toLowerCase();
  return keywords.some(k => text.includes(k));
};

const isBudgetStay = (rent: number) => rent <= 5000;
const isBestValue = (rent: number, sharingType: string) => {

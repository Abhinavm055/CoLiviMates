...
import { Home, Building, Users, Shield, ArrowLeft, Loader2 } from 'lucide-react';
...
export default function Auth() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!success) {
      toast({
        title: 'Login Failed',
        description: 'Invalid email or password. Try using: admin@coliving.com, owner@example.com, or tenant@example.com',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleRoleSelect = (role: UserRole) => {
    ...
    toast({
      title: 'Welcome!',
      description: `Your account has been created as ${role.replace('_', ' ')}.`,
    });
  };
      ...
      <Layout>
        <div className="container mx-auto px-4 py-16 max-w-2xl">
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground">Select how you want to use CoLiveMates</p>
          </div>
          ...
          <div className="grid gap-4">
            {roles.map(({ role, title, description, icon }) => (
              <button
                ...
                className="bg-card rounded-xl p-6 card-elevated text-left flex items-center gap-5 hover:border-primary border-2 border-transparent transition-all group"
              >
            ...
            {mode === 'login' && (
              <div className="mt-6 p-4 bg-muted rounded-lg">
                ...
                <ul className="text-xs text-muted-foreground space-y-1">
                  <li><strong>Admin:</strong> admin@coliving.com</li>
